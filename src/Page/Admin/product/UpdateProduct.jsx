...
const UpdateProduct = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        const e = [];
        response[0].product_entries.map(async (item) => {
          await e.push({ quantity: item.quantity, sizeId: item.sizeId });
        });
        console.log('entry',e)
        ...
        //   fields: e,
          product_entrys:e
        }); // Đặt giá trị mặc định cho trường name trong Form
        const i = [];
  ...
  return (
    <Form form={form} onFinish={onFinish} layout="vertical">
      ...
      <Form.Item name="upload" label="Ảnh">
        <Upload
